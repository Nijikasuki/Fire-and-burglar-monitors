...
void RateCube_all(float x,float y,float z,float mx,float my,float mz,float bx,float by,float bz,int16_t XO,int16_t YO,uint32_t color);
void RotatePic32X32(unsigned char *dp,float ax,float ay,float az,uint32_t color,uint16_t XO,uint16_t YO);//在三维空间中旋转一个32x32的字符或图形
